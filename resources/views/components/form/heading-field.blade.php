...
<div class="flex justify-start items-center w-full gap-4">
    ...
    @if ($type === 'editable')
        <input
            ...
            class="{{ $sizeClass }} w-full border-0 px-3 py-2 rounded-md bg-transparent transition duration-150 ease-in-out focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
            {{ $disabled ? 'disabled' : '' }}
            {{ $required ? 'required' : '' }}
            {{ $readonly ? 'readonly' : '' }}
        />
