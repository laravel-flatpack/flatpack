...
import { useSafeComboboxInput } from '@/hooks/use-safe-combobox-input';
...
const defaultFilter: FilterFn = (
    ...
    return Array.from(uniqueTerms).some(
        (keyword) => keyword !== undefined && filterWords(keyword, search),
    );
};
...
const InlineCombobox = ({
    ...
    const isCreator = React.useMemo(() => {
        const elementUserId = (element as TElement & { userId?: string })
            .userId;
    ...
    const { props: inputProps, removeInput } = useSafeComboboxInput({
...
const InlineComboboxInput = ({
    ...
    if (!store) {
        throw new Error(
            'InlineComboboxInput must be used inside InlineCombobox',
        );
    }
...
const InlineComboboxItem = ({
    ...
    if (!store) {
        throw new Error(
            'InlineComboboxItem must be used inside InlineCombobox',
        );
    }
...
const InlineComboboxEmpty = ({
    ...
    if (!store) {
        throw new Error(
            'InlineComboboxEmpty must be used inside InlineCombobox',
        );
    }
    const items = store.useState('items');
