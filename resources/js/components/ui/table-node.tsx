    ...
    const applyResize = React.useCallback(
        (event: PointerEvent, finished: boolean) => {
            ...
            if (dragState.direction === 'left') {
                ...
                if (finished) {
                    ...
                    showDeferredResizeIndicator(
                        controlColumnWidth +
                            (nextMarginLeft - dragState.marginLeft),
                    );
                } else {
            ...
            const currentInitial =
                ...
                dragState.initialSize;
            const nextInitial =
                effectiveColSizesRef.current[dragState.colIndex + 1];
            const complement = (width: number) =>
            ...
            const nextWidth = nextInitial
                ? complement(currentWidth)
                : undefined;
    ...
    const startResize = React.useCallback(
        (
            event: React.PointerEvent<HTMLDivElement>,
            {
                ...
                handleKey,
                rowIndex,
            }: TableResizeStartOptions,
        ) => {
            const rowHeight =
                tableRef.current?.rows.item(rowIndex)?.getBoundingClientRect()
                    .height ?? 0;
...
function TableFloatingToolbar({
    ...
    const [
        isExpandedSelectionToolbarReady,
        setIsExpandedSelectionToolbarReady,
...
export function TableRowElement({
    ...
    const { isDragging, nodeRef, previewRef, handleRef } = useDraggable({
        ...
        onDropHandler: (
            ...
            const dragElement =
                'element' in dragItem ? dragItem.element : undefined;
    ...
    return (
        <PlateElement
            ...
            style={
                {
                    ...props.style,
                    '--tableRowMinHeight': rowMinHeight
                        ? `${rowMinHeight}px`
                        : undefined,
                } as React.CSSProperties
            }
