...
import { getPluginByType, isType, KEYS, PathApi, type TElement } from 'platejs';
import {
    MemoizedChildren,
    type PlateEditor,
...
function BlockInsertButton({ element }: { element: TElement }) {
        ...
        <Tooltip>
            <TooltipTrigger asChild>
                <Button
                    variant="ghost"
                    size="icon"
                    className="relative size-5 shrink-0 p-0"
                    type="button"
                    onMouseDown={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        ...
                        queueMicrotask(() => {
                            ...
                            const at = PathApi.next(blockPath);
        ...
        >
            {!isInTable && (
                <Gutter>
                    ...
                    >
                        <BlockInsertButton element={element} />
                        <div
                            className={cn(
                                'slate-blockToolbarWrapper flex items-center',
                                isInColumn && 'min-h-4',
                            )}
                        >
                            <div
                                className={cn(
                                    'slate-blockToolbar relative flex w-4.5 shrink-0 items-center justify-center',
                                    'pointer-events-auto',
                                    isInColumn && 'w-5',
                                )}
                            >
...
function Gutter({
    ...
    return (
        <div
            ...
            className={cn(
                'slate-gutterLeft',
                'absolute top-0 z-50 flex h-full cursor-text hover:opacity-100 sm:opacity-0',
                'right-full mr-1',
                getPluginByType(editor, element.type)?.node.isContainer
