...
import { menuIcons } from './lucide-menu-icon-registry';
...
function resolveMenuIconKey(name: string): keyof typeof menuIcons | null {
    const trimmed = name.trim();
    if (!trimmed) {
        return null;
    }

    if (Object.hasOwn(menuIcons, trimmed)) {
        return trimmed as keyof typeof menuIcons;
    }

    const kebab = normalizeToLucideKebabCase(trimmed);
    if (Object.hasOwn(menuIcons, kebab)) {
        return kebab as keyof typeof menuIcons;
    }
